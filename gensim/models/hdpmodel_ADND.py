...
class HdpModel(interfaces.TransformationABC, basemodel.BaseTopicModel):
    ...
    * whole corpus processed
    ...
    def __init__(self, corpus, id2word, max_chunks=None, max_time=None,
        ...
        # self.m_var_sticks_sender = np.zeros((2, K - 1))
        # self.m_var_sticks_sender[0] = 1.0
        # self.m_var_sticks_sender[1] = range(K - 1, 0, -1)
        # self.m_varphi_ss_sender = np.zeros(K)
        # self.m_var_sticks_receiver = np.zeros((2, K - 1))
        # self.m_var_sticks_receiver[0] = 1.0
        # self.m_var_sticks_receiver[1] = range(K - 1, 0, -1)
        # self.m_varphi_ss_receiver = np.zeros(K)
        self.m_varphi_sender = np.zeros((K, T))
        self.m_sticks_sender = np.zeros(K)
        self.m_varphi_receiver = np.zeros((K, T))
        self.m_sticks_receiver = np.zeros(K)

        # self.m_lambda = self.random_state.gamma(1.0, 1.0, (T, self.m_W)) * self.m_D * 100 / (T * self.m_W) - eta
        self.m_lambda = self.random_state.gamma(1.0, 1.0, (T, self.m_W)) / (T * self.m_W) - eta
        self.m_eta = eta
    ...
    def inference_new_word(self, word, sender=True):
        # if word in self.id2word.keys():
        #     ids = self.id2word[word]
        # else:
        #     ids = -1
        # counts = 1
        ...
        # if word in self.id2word.keys():
        #     unique_words = {self.id2word[word]: 1}
        #     doc_word_ids = [self.id2word[word]]
        # else:
        #     unique_words = {-1: 1}
        #     doc_word_ids = [-1]
        # doc_word_counts = 1
        # return self.doc_e_step_trained(expect_log_sticks(self.m_var_sticks), unique_words, doc_word_ids, doc_word_counts, self.m_var_converge)

        if word in self.id2word.keys():
            word_id = self.id2word[word]
        else:
            word_id = -1
        if sender:
            return (np.exp(self.m_sticks_sender.reshape(1, -1)) @ self.m_varphi_sender @ self.m_Elogbeta)[0, word_id]
        else:
            return (np.exp(self.m_sticks_receiver.reshape(1, -1)) @ self.m_varphi_receiver @ self.m_Elogbeta)[0, word_id]
    ...
    def update(self, corpus, sender=True):
        ...
        * whole corpus processed
        ...
        while True:
            for chunk in utils.grouper(corpus, self.chunksize):
                self.update_chunk(chunk, sender=sender)
    ...
    def update_chunk(self, chunk, sender=True, update=True, opt_o=True):
    ...
    def doc_e_step(self, Elogsticks_1st, unique_words, doc_word_ids, doc_word_counts, var_converge, sender=True):
        ...
        while iter < max_iter and (converge <= 0.0 or converge > var_converge):
            ...
            print(f"log likelihood updated by {converge}")
        ...
        if sender:
            self.m_sticks_sender = Elogsticks_2nd
            self.m_varphi_sender = var_phi
        else:
            self.m_sticks_receiver = Elogsticks_2nd
            self.m_varphi_receiver = var_phi
